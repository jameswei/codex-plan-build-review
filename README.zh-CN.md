...
对于实现工作，主 agent 选择最小且足够的审查 charter，例如实现行为、最终公共表面或跨任务集成。涉及异步、并发、轮询或其他时间敏感型协调行为的工作，必须说明时序不变量和消费者可观察行为；实现和审查再据此使用有界的场景矩阵。一个任务必须是可独立合并和验证的语义完整单元。工作流不按文件类型或代码层机械拆分任务。每个 verdict 还会说明采用的 charter、实际审查范围和覆盖限制。
